...
function getArtistByIndex(a,i) {
  return `the artist at index ${i} is ${a[i].name}`;
}
...
function addArtist(a,n,y,g,c,b) {
  a.push({
    name: n, 
    years: y,
    genre: g, 
...
function lotsOfArt(a) {
  let ar=[];
  for(let v of a){
    if(v.paintings>100) ar.push(v.name)
}
return ar;
}
...
function artistByCountry(a,c){
  let ar=[];
  for(let v of a){
    if(v.nationality===c) ar.push(v.name);
  }
  return ar;
}
